...
def _build_orchestrator(request, llm_config=None):
    ...
    from cortex_engine.infrastructure.change_detector import ChangeDetector
    from cortex_engine.infrastructure.hygiene_checker import HygieneChecker
    from cortex_engine.infrastructure.git import GitOperations

    from cortex_engine.tools.runner import Tier1Runner
    from cortex_engine.assessment.risk_scorer import RiskScorer
    from cortex_engine.assessment.quality_gate import QualityGate
    from cortex_engine.reporting.report_generator import ReportGenerator
    from cortex_engine.reporting.executive_report import ExecutiveReportGenerator
    from cortex_engine.integrations.dispatcher import IntegrationDispatcher
    from cortex_engine.core.finding_manager import FindingManager

    from cortex_engine.orchestration.orchestrator import ScanOrchestrator
    from cortex_engine.orchestration.cost_tracker import CostTracker
    ...
    if request.post_comment:
        from cortex_engine.integrations.github import GitHubIntegration
    ...
    finding_repo = None
    try:
        from cortex_engine.infrastructure.database import get_connection, init_db
        from cortex_engine.infrastructure.scan_repository import SQLiteScanRepository
        from cortex_engine.infrastructure.finding_repository import SQLiteFindingRepository
...
def _register_tools(runner):
    ...
    tool_classes = []
    try:
        from cortex_engine.tools.ruff_tool import RuffTool
        tool_classes.append(RuffTool)
    except ImportError:
        pass
    try:
        from cortex_engine.tools.bandit_tool import BanditTool
        tool_classes.append(BanditTool)
    except ImportError:
        pass
    try:
        from cortex_engine.tools.mypy_tool import MypyTool
        tool_classes.append(MypyTool)
    except ImportError:
        pass
    try:
        from cortex_engine.tools.semgrep_tool import SemgrepTool
        tool_classes.append(SemgrepTool)
    except ImportError:
        pass
    try:
        from cortex_engine.tools.radon_tool import RadonTool
        tool_classes.append(RadonTool)
    except ImportError:
        pass
    try:
        from cortex_engine.tools.security_patterns import SecurityPatternsTool
        tool_classes.append(SecurityPatternsTool)
    except ImportError:
        pass
    try:
        from cortex_engine.tools.complexity_tool import ComplexityTool
        tool_classes.append(ComplexityTool)
    except ImportError:
        pass
    try:
        from cortex_engine.tools.shellcheck_tool import ShellcheckTool
        tool_classes.append(ShellcheckTool)
    except ImportError:
        pass
    try:
        from cortex_engine.tools.gitleaks_tool import GitleaksTool
    ...
    for tool_ref in optional_tools:
        ...
        try:
            mod = __import__(f"cortex_engine.tools.{module_name}", fromlist=[class_name])
...
def _build_agent_infrastructure(request, cost_tracker, llm_config=None):
    ...
    from cortex_engine.orchestration.review_engine import AgenticReviewEngine
    from cortex_engine.orchestration.validation_engine import ValidationEngine
